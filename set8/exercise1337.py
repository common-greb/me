...
def list_please() -> list:
    ...
    return [1, 'apple', True, [2, 3], {'name': 'John'}]
...
def set_it_on_fire(input_string="very naughty boy") -> str:
    ...
    interleave_list = [f"{char}🔥" for char in input_string]
    return "🔥" +''.join(interleave_list)
...
def best_letter_for_pets() -> str:
    ...
    letter_counts = {letter: len(pet_filter(letter)) for letter in alphabet}
...
def make_filler_text_dictionary() -> Dict:
    ...
    url_template = "https://us-central1-waldenpondpress.cloudfunctions.net/give_me_a_word?wordlength={length}"
    filler_text_dict = {}

    for length in range(3, 8):
        url = url_template.format(length=length)
        words = [requests.get(url).text for _ in range(4)]
        filler_text_dict[length] = words
...
def random_filler_text(number_of_words=200) -> str:
    ...
    words = [random.choice(my_dict[random.randint(3, 7)]) for _ in range(number_of_words)]
...
def fast_filler(number_of_words=200) -> str:
    ...
    if os.path.exists(fname):
        with open(fname, "r") as file:
            filler_text_dict = json.load(file)
    else:
        filler_text_dict = make_filler_text_dictionary()
        with open(fname, "w") as file:
            json.dump(filler_text_dict, file)

    words = [random.choice(filler_text_dict[str(random.randint(3, 7))]) for _ in range(number_of_words)]
    return " ".join(words)
